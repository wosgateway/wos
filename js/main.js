...
(function() {
    ...
    // Fetch all packages belonging to active partners in given category(ies)
    // e.g. fetchPackagesByCategory(['Hotel']) — used by the optional
    // hotel/transport package pickers in booking.html
    window.fetchPackagesByCategory = async function(dbCategories) {
        if (!window.__supabase) throw new Error('Supabase client is not initialized');
        const { data, error } = await window.__supabase
            .from('packages')
